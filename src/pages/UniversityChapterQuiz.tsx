...
import { ChevronLeft, CheckCircle, XCircle, RotateCcw, Lock } from 'lucide-react';
...
export default function UniversityChapterQuiz() {
  ...
  const alreadyPassed = hasPassedChapterQuiz(levelNum, unitNum, chapterNum);
  ...
  const handleSubmit = () => {
    ...
    submitChapterQuiz.mutate(
      { level: levelNum, unitNumber: unitNum, chapterNumber: chapterNum, score: correct, total, passed, answers: answers as number[] },
      {
        onSuccess: () => {
          if (passed) {
            toast.success('Quiz passed! Next chapter unlocked.');
          } else {
            toast('Not quite — try again with new questions.');
          }
        },
      }
    );
  ...
  const handleContinue = () => {
    ...
    if (chapterNum < totalChapters) {
      ...
      navigate(`/university/level-${levelNum}`);
    }
  };
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="pt-24 pb-4 border-b border-primary/20">
          <div className="container mx-auto px-4 max-w-2xl">
            <Button variant="ghost" size="sm" onClick={() => navigate(`/university/level-${levelNum}/unit-${unitNum}/chapter-${chapterNum}`)} className="mb-2 -ml-2 text-muted-foreground">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="max-w-2xl mx-auto space-y-6">
            <Card className={`p-6 text-center border-2 ${passed ? 'border-green-500/50 bg-green-500/10' : 'border-destructive/50 bg-destructive/10'}`}>
              ...
              {!passed && (
                <p className="text-muted-foreground text-xs">Review the chapter content and try again. You'll get different questions next time.</p>
              )}
            </Card>
